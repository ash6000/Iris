...
class VoiceEntryViewController_MVVM: UIViewController {
    private let viewModel = VoiceEntryViewModel()
    ...
    private func bindViewModel() {
        viewModel.$isRecording.bind(self) { [weak self] isRecording in
            DispatchQueue.main.async {
                self?.isRecording = isRecording
            }
        }

        viewModel.$recordingTime.bind(self) { [weak self] time in
            DispatchQueue.main.async {
                self?.recordingLabel.text = "Recording: \(time)"
            }
        }
