...
class PersonalizedChatViewController: UIViewController {
    ...
    private var currentPlayingBubble: AudioMessageBubble?
    ...
    private func setupInputContainer() {
        ...
        // Voice Mode Toggle
        voiceModeToggle.translatesAutoresizingMaskIntoConstraints = false
        voiceModeToggle.setImage(UIImage(systemName: "mic"), for: .normal)
        voiceModeToggle.setImage(UIImage(systemName: "mic.fill"), for: .selected)
        voiceModeToggle.tintColor = UIColor(red: 0.5, green: 0.5, blue: 0.5, alpha: 1.0)
    ...
    private func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            // Voice Mode Toggle
            voiceModeToggle.leadingAnchor.constraint(equalTo: inputContainerView.leadingAnchor, constant: 16),
            voiceModeToggle.centerYAnchor.constraint(equalTo: inputBackgroundView.centerYAnchor),
    ...
    private func showMicrophonePermissionAlert() {
        let alert = UIAlertController(
            ...
            message: "To use voice features, please enable microphone access in Settings.",
...
extension PersonalizedChatViewController: AudioMessageBubbleDelegate {
    func audioMessageBubbleDidTapPlay(_ bubble: AudioMessageBubble) {
        ...
        if let currentBubble = currentPlayingBubble {
            currentBubble.setPlayingState(false)
            VoiceService.shared.stopPlayback()
        }

        // Start playing this audio
        if let audioData = bubble.getAudioData() {
            currentPlayingBubble = bubble
            bubble.setPlayingState(true)
